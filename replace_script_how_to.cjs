...
const newComponentPath = path.join(process.cwd(), 'new_how_to_create.jsx');

const appJsx = fs.readFileSync(appPath, 'utf8');
const replacement = fs.readFileSync(newComponentPath, 'utf8');

const lines = appJsx.split('\n');
const startIdx = lines.findIndex(l => l.startsWith('function PageHowToCreate() {'));
const endIdx = lines.findIndex((l, i) => i > startIdx && l.startsWith('function PageRunAndTest() {'));
