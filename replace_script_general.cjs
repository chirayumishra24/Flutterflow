...
const newComponentPath = path.join(process.cwd(), 'new_general_settings.jsx');

const appJsx = fs.readFileSync(appPath, 'utf8');
const replacement = fs.readFileSync(newComponentPath, 'utf8');

const lines = appJsx.split('\n');
const startIdx = lines.findIndex(l => l.startsWith('function PageGeneralSettings() {'));
const endIdx = lines.findIndex((l, i) => i > startIdx && l.startsWith('function PageProjectSetup() {'));
